...
import { ChevronLeft, FolderPlus, Loader2, RefreshCw, Sparkles } from "lucide-react";
    ...
    >
      ...
      <CardContent className="grid w-full grid-cols-1 content-start gap-y-4 px-4 pb-5 pt-0 sm:grid-cols-2 sm:gap-x-4">
        <div className="flex flex-col gap-1.5 sm:col-span-2">
          <Label htmlFor="systemOrProduct" className="text-sm font-medium">
            Sistema ou produto relacionado
          </Label>
          <Input
            id="systemOrProduct"
            placeholder="Ex.: Portal do cliente"
            value={formData.systemOrProduct}
            onChange={(e) =>
              setFormData((prev) => ({ ...prev, systemOrProduct: e.target.value }))
            }
            className="h-9"
          />
        </div>
        ...
        <div className="flex flex-col gap-1.5 sm:col-span-2">
          ...
          <p className="text-sm text-muted-foreground">
            Obrigatório para análise e salvamento da demanda.
          </p>
          {isProjectsLoading ? (
            ...
            <div className="flex flex-col gap-2">
              <p className="text-sm text-muted-foreground">
                {projectsError ?? "Nenhum projeto foi retornado pela API."}
              </p>
        ...
        <div className="flex flex-col gap-1.5 sm:col-span-2">
          ...
          <Input
            id="requester"
            placeholder="Ex.: Product owner, suporte, usuário final"
          ...
          />
          <p className="text-sm text-muted-foreground">
            Esse valor será enviado como `perfil_solicitante` para a API.
          </p>
        </div>
